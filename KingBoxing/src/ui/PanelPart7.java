...
public class PanelPart7 extends JPanel implements MouseListener{
	GameFrame gameFrame;
	JLabel[] lab=new JLabel[30];
	//Thread thread1;
	
	
	
    public PanelPart7(GameFrame gameFrame){
    ...
    public void labelInitionalization(){
    	for(int i=0;i<=29;i++){
			this.lab[i]=new JLabel(gameFrame.images.story7Label[i]);
		}
		for(int j=0;j<=29;j++){
    ...
    public void paintComponent(Graphics g){
		g.drawImage(gameFrame.images.part7_1, 0, 0, GameFrame.JFRAME_WIDTH, GameFrame.JFRAME_HEIGHT,this);
	}
	@Override
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getSource()==lab[18]){
			...
			this.add(lab[19]);
//			thread1.stop();
//			thread1=new Thread(gameFrame.music7);
//			thread1.start();
			this.repaint();
		}
		...
		if(e.getSource()==lab[29]){
			this.remove(lab[29]);
			ChooseSth7 chooseSth7=new ChooseSth7(gameFrame);
			JPanel panel=(JPanel) gameFrame.getContentPane();
			gameFrame.remove(panel);
			gameFrame.setContentPane(chooseSth7);
